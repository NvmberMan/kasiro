<x-tenant-page>
    ...
    {{-- Welcome --}}
    <div class="mb-6">
        <h1 class="text-2xl font-bold">Selamat datang di {{ $tenant->name }}</h1>
        <p class="text-sm opacity-70 mt-1">
            Halo, {{ $user?->name ?? 'Pengguna' }}@if ($role) &middot; <span class="capitalize">{{ $role->value }}</span>@endif
        </p>
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        @if ($canViewReports && $stats)
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
                <p class="text-xs font-medium text-gray-500">Transaksi Hari Ini</p>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
            ...
            @if ($lowStock > 0)
                <p class="text-xs text-amber-600 mt-1">{{ $lowStock }} produk stok menipis</p>
            @endif
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
        <a href="{{ route('tenant.pos', ['subdomain' => $sub]) }}"
           class="group bg-white rounded-xl border border-gray-100 shadow-sm p-5 hover:border-indigo-300 hover:shadow transition flex flex-col gap-2">
            ...
            <span class="font-semibold text-gray-800">Buka Kasir</span>
            <span class="text-xs text-gray-500">Mulai transaksi penjualan</span>
        </a>
        ...
        <a href="{{ route('tenant.products.index', ['subdomain' => $sub]) }}"
           class="group bg-white rounded-xl border border-gray-100 shadow-sm p-5 hover:border-indigo-300 hover:shadow transition flex flex-col gap-2">
            ...
            <span class="font-semibold text-gray-800">Kelola Produk</span>
            <span class="text-xs text-gray-500">Tambah & ubah produk</span>
        </a>
        ...
        <a href="{{ route('tenant.reports', ['subdomain' => $sub]) }}"
           class="group bg-white rounded-xl border border-gray-100 shadow-sm p-5 hover:border-indigo-300 hover:shadow transition flex flex-col gap-2">
            ...
            <span class="text-xs text-gray-500">Ringkasan penjualan</span>
        </a>
        <a href="{{ route('tenant.transactions', ['subdomain' => $sub]) }}"
           class="group bg-white rounded-xl border border-gray-100 shadow-sm p-5 hover:border-indigo-300 hover:shadow transition flex flex-col gap-2">
            ...
            <span class="font-semibold text-gray-800">Transaksi</span>
            <span class="text-xs text-gray-500">Riwayat transaksi</span>
        </a>
        @endif
        ...
        <a href="{{ route('tenant.settings.edit', ['subdomain' => $sub]) }}"
           class="group bg-white rounded-xl border border-gray-100 shadow-sm p-5 hover:border-indigo-300 hover:shadow transition flex flex-col gap-2">
            ...
            <span class="text-xs text-gray-500">Karyawan & toko</span>
        </a>
        @endif
    </div>
    ...
    @if ($canViewReports)
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            <div class="flex items-center justify-between px-5 py-3 border-b border-gray-100">
                ...
                <a href="{{ route('tenant.transactions', ['subdomain' => $sub]) }}"
                   class="text-xs font-medium brand-text hover:underline">Lihat semua</a>
            </div>
            @if ($recent->isEmpty())
                ...
                <table class="w-full text-sm">
                    <thead class="text-left text-xs text-gray-500 bg-gray-50">
                        <tr>
                            ...
                            <th class="px-5 py-2 font-medium">Kasir</th>
                            <th class="px-5 py-2 font-medium">Metode</th>
