...
<body class="antialiased h-screen flex overflow-hidden"
    ...
    {{-- Brand rail --}}
    <aside class="brand-primary text-white flex flex-col flex-shrink-0 transition-all duration-200"
        ...
        <a href="{{ route('tenant.home', ['subdomain' => $sub]) }}"
           ...
           aria-label="Beranda {{ $tenant->name }}">
            ...
            @endif
            <span class="font-bold text-sm truncate" :class="open ? '' : 'hidden'">{{ $tenant->name }}</span>
        </a>
        ...
        <nav class="flex-1 overflow-y-auto py-3 space-y-1">
            ...
            <a href="{{ route('dashboard') }}" class="{{ $link }} mt-2">
                ...
                <span :class="open ? '' : 'hidden'">Studio</span>
            </a>
        </nav>
        ...
        <div class="px-3 py-3 border-t border-white/15 flex items-center gap-2">
            ...
            <button @click="open = !open" class="flex-shrink-0 text-white/60 hover:text-white transition" aria-label="Lipat menu">
