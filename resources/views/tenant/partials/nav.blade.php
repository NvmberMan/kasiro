@php
    ...
    $base    = 'flex items-center gap-2 px-3.5 py-2 text-sm font-medium rounded-full whitespace-nowrap transition';
    $idle    = 'brand-muted hover:text-[color:var(--brand-primary)]';
    $on      = 'brand-soft brand-text';
@endphp
<nav class="brand-surface border-b brand-border">
    <div class="mx-auto max-w-7xl px-4 flex items-center gap-1 overflow-x-auto py-2">
        <a href="{{ route('tenant.pos', ['subdomain' => $sub]) }}"
           class="{{ $base }} {{ request()->routeIs('tenant.pos') ? $on : $idle }}">
            Kasir
        </a>
        <a href="{{ route('tenant.products.index', ['subdomain' => $sub]) }}"
           class="{{ $base }} {{ request()->routeIs('tenant.products.*') ? $on : $idle }}">
            Produk
        </a>
        <a href="{{ route('tenant.categories.index', ['subdomain' => $sub]) }}"
           class="{{ $base }} {{ request()->routeIs('tenant.categories.*') ? $on : $idle }}">
            Kategori
        </a>
        @if ($navRole?->canViewReports())
            <a href="{{ route('tenant.reports', ['subdomain' => $sub]) }}"
               class="{{ $base }} {{ request()->routeIs('tenant.reports') ? $on : $idle }}">
                Laporan
            </a>
            <a href="{{ route('tenant.transactions', ['subdomain' => $sub]) }}"
               class="{{ $base }} {{ request()->routeIs('tenant.transactions') ? $on : $idle }}">
                Transaksi
            </a>
        @endif
        @if ($navRole?->canManageStaff())
            <a href="{{ route('tenant.employees.index', ['subdomain' => $sub]) }}"
               class="{{ $base }} {{ request()->routeIs('tenant.employees.*') ? $on : $idle }}">
                Karyawan
            </a>
            <a href="{{ route('tenant.settings.edit', ['subdomain' => $sub]) }}"
               class="{{ $base }} {{ request()->routeIs('tenant.settings.*') ? $on : $idle }}">
                Pengaturan
            </a>
        @endif

        <a href="{{ route('dashboard') }}" class="ml-auto {{ $base }} {{ $idle }}">
