                            ...
                            <div class="flex items-center justify-between bg-white rounded-xl border border-gray-200 shadow-sm px-6 py-4 gap-4"
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="text-sm text-gray-400 mt-0.5">
                                        Diarsipkan {{ $tenant->archived_at?->format('d/m/Y') }}
                                    </p>
                                </div>
                                ...
                                <div class="flex items-center gap-2 shrink-0">
                                    <form method="POST" action="{{ route('tenants.restore', $tenant) }}"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                                class="text-sm font-medium text-indigo-600 border border-indigo-400 rounded-full px-4 py-1.5 hover:bg-indigo-50 transition-colors">
                                            Pulihkan
                                        </button>
                                    </form>

                                    <form method="POST" action="{{ route('tenants.destroy', $tenant) }}"
                                          data-confirm="Toko dan semua datanya akan dihapus permanen dan tidak bisa dikembalikan."
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                                class="text-sm font-medium text-red-600 border border-red-400 rounded-full px-4 py-1.5 hover:bg-red-50 transition-colors">
                                            Hapus
                                        </button>
                                    </form>
                                </div>
